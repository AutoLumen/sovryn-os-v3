...
const SECRET_PATTERNS: Array<{ name: string; regex: RegExp }> = [
  { name: "private-key", regex: /-----BEGIN (?:RSA |OPENSSH |EC |DSA )?PRIVATE KEY-----[\s\S]*?-----END (?:RSA |OPENSSH |EC |DSA )?PRIVATE KEY-----/gi },
  ...
  { name: "password-assignment", regex: /\b(password|passwd|pwd)\s*[:=]\s*["']?[^"'\s]{4,}/gi },
  { name: "api-key-assignment", regex: /\b(api[_-]?key|secret|token|credential)\s*[:=]\s*["']?[^"'\s]{8,}/gi }
];
