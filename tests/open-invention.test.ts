...
test("invent-open creates an open invention mission and dossier files", async () => {
  ...
  const response = await executeCli(["invent-open", "A method for verifiable open-source agent research", "--json"], repo.root);
  ...
  await access(join(repo.root, mission.inventionPath, "DEFENSIVE_PUBLICATION.md"));
  await access(join(repo.root, mission.inventionPath, "PRIOR_ART.md"));
  await access(join(repo.root, mission.inventionPath, "LICENSE"));
  await access(join(repo.root, mission.inventionPath, "prototype", "tests", "prototype.test.js"));
  ...
  assert.equal(dossier.priorArtMatrix.some((item: any) => item.sourceType === "patent"), true);
...
test("publication review blocks missing license", async () => {
  ...
  const review = await executeCli(["invention", "review", mission.id, "--json"], repo.root);
...
test("publication review blocks failing prototype tests", async () => {
  const { repo, mission } = await createOpenInvention();
  await writeFile(join(repo.root, mission.inventionPath, "prototype", "tests", "prototype.test.js"), "process.exit(1);\n", "utf8");
  const review = await executeCli(["invention", "review", mission.id, "--json"], repo.root);
  assert.equal(review.ok, true);
  const result = (review.data as any).review;
  assert.equal(result.allowed, false);
...
test("publication review blocks secrets in generated invention files", async () => {
  ...
  await writeFile(join(repo.root, mission.inventionPath, "prototype", "generated-secret.txt"), `token=${fakeToken}\n`, "utf8");
  const review = await executeCli(["invention", "review", mission.id, "--json"], repo.root);
  assert.equal(review.ok, true);
  const result = (review.data as any).review;
  assert.equal(result.allowed, false);
  assert.equal(checkPassed(result, "SECRET_SCAN"), false);
  assert.equal(result.secretFindings.some((finding: any) => finding.location === "prototype/generated-secret.txt"), true);
});

test("publication review blocks unsafe generated content", async () => {
  const { repo, mission } = await createOpenInvention();
  await writeFile(join(repo.root, mission.inventionPath, "prototype", "unsafe.txt"), "malware payload instructions\n", "utf8");
  const review = await executeCli(["invention", "review", mission.id, "--json"], repo.root);
  assert.equal(review.ok, true);
  const result = (review.data as any).review;
  assert.equal(result.allowed, false);
  assert.equal(checkPassed(result, "SAFETY_SCAN"), false);
  assert.equal(result.safetyFindings.some((finding: any) => finding.path === "prototype/unsafe.txt"), true);
});

test("publish-github dry-run reruns final verification and writes publication evidence", async () => {
  const { repo, mission } = await createOpenInvention();
  const finalize = await executeCli(["invention", "finalize", mission.id, "--json"], repo.root);
  assert.equal(finalize.ok, true);
  const finalVerifyPath = join(repo.root, mission.inventionPath, "evidence", "final-verify.json");
  const before = JSON.parse(await readFile(finalVerifyPath, "utf8"));
  await new Promise((resolve) => setTimeout(resolve, 20));
  await writeFile(join(repo.root, mission.inventionPath, "README.md"), `${await readFile(join(repo.root, mission.inventionPath, "README.md"), "utf8")}\nAdditional validation note.\n`, "utf8");
  const publish = await executeCli(["publish-github", mission.id, "--dry-run", "--json"], repo.root);
...
test("publication review blocks tests that mutate publication source", async () => {
  const { repo, mission } = await createOpenInvention();
  await writeFile(
    join(repo.root, mission.inventionPath, "prototype", "tests", "prototype.test.js"),
...
test("publish-github blocks non-finalized real publication", async () => {
  const { repo, mission } = await createOpenInvention();
  const publish = await executeCli(["publish-github", mission.id, "--org", "example", "--repo", "demo", "--json"], repo.root);
...
test("Node Alpha local backend creates workspace logs and artifacts", async () => {
  const { repo, mission } = await createOpenInvention();
  const register = await executeCli(["node", "register", "alpha", "--host", "local", "--json"], repo.root);
  assert.equal(register.ok, true);
  assert.equal((register.data as any).registration.backend, "local");
  const status = await executeCli(["node", "status", "alpha", "--json"], repo.root);
  assert.equal(status.ok, true);
  assert.equal((status.data as any).registration.id, "alpha");
  const run = await executeCli(["node", "run", "alpha", mission.id, "--json"], repo.root);
  ...
  const logs = await executeCli(["node", "logs", "alpha", mission.id, "--json"], repo.root);
...
test("Node Alpha autonomous mode writes plan journal scores and research artifacts", async () => {
  ...
  const score = JSON.parse(await readFile(join(evidenceDir, "artifact-score.json"), "utf8"));
...
test("GitHub dry-run stages only public evidence", async () => {
  const { repo, mission } = await createOpenInvention();
  await executeCli(["node", "register", "alpha", "--host", "local"], repo.root);
  await executeCli(["node", "run", "alpha", mission.id, "--mode", "autonomous"], repo.root);
  const finalize = await executeCli(["invention", "finalize", mission.id, "--json"], repo.root);
  assert.equal(finalize.ok, true);
  const publish = await executeCli(["publish-github", mission.id, "--dry-run", "--json"], repo.root);
  assert.equal(publish.ok, true);
  const releasePath = (publish.data as any).publication.releasePath;
  await access(join(releasePath, "evidence", "public", "command-journal.redacted.json"));
  await access(join(releasePath, "evidence", "public", "final-verify.summary.json"));
  await assert.rejects(access(join(releasePath, "evidence", "command-logs")));
  await assert.rejects(access(join(releasePath, "evidence", "command-journal.json")));
});
...
function checkPassed(result: any, code: string): boolean | null {
  return result.checks.find((check: any) => check.code === code)?.passed ?? null;
}
