...
import type { CommandJournal, CommandJournalEntry, NodeArtifactIndex, NodeEnvironment, NodeRegistration, NodeRunOptions, NodeRunResult, ResearchPlan, ResearchPlanStep } from "./node-types.js";
...
export interface NodeAlphaBackend {
  ...
  runOpenInvention(mission: OpenInventionMissionState, options?: NodeRunOptions): Promise<NodeRunResult>;
...
export class LocalNodeAlphaBackend implements NodeAlphaBackend {
  ...
  async runOpenInvention(mission: OpenInventionMissionState, options: NodeRunOptions = { mode: "validation", maxSteps: 25 }): Promise<NodeRunResult> {
    ...
    for (const step of boundedSteps) {
      const cwd = step.cwd === "prototype" ? workspacePrototypePath : workspaceInventionPath;
      assertNodeCommandAllowed(step.command);
      step.status = "running";
      step.startedAt = nowIso();
      const result = await runCommand(step.command, cwd, { allowNetwork: step.allowNetwork });
      step.completedAt = nowIso();
      step.exitCode = result.exitCode;
      step.status = result.exitCode === 0 ? "completed" : "failed";
      const stdoutPath = join(workspaceInventionPath, "evidence", "command-logs", `${step.id}.stdout.txt`);
      const stderrPath = join(workspaceInventionPath, "evidence", "command-logs", `${step.id}.stderr.txt`);
      await mkdir(join(workspaceInventionPath, "evidence", "command-logs"), { recursive: true });
    ...
    for (const step of plan.steps.slice(boundedSteps.length)) step.status = "skipped";
    ...
    const planPath = join(workspaceInventionPath, "evidence", "research-plan.json");
    const journalPath = join(workspaceInventionPath, "evidence", "command-journal.json");
    ...
    await cp(join(workspaceInventionPath, "evidence"), join(sourcePath, "evidence"), { recursive: true, force: true });
...
function createResearchPlan(nodeId: string, mission: OpenInventionMissionState, options: NodeRunOptions): ResearchPlan {
  const createdAt = nowIso();
  const autonomousSteps = [
    planStep("inspect-node", "brief", "Inspect Node Alpha runtime", "Record local toolchain versions for reproducibility.", "node --version && npm --version && git --version", "invention"),
    planStep("map-landscape", "landscape_scan", "Create landscape scan", "Write deterministic landscape notes for later public-source enrichment.", writeFileCommand("evidence/landscape-scan.md", `# Landscape Scan\n\nMission: ${mission.title}\n\nThis autonomous MVP records a local landscape scaffold. Future providers should enrich this with public web, GitHub, arXiv, OpenAlex, standards, and documentation searches.\n`), "invention"),
    planStep("map-prior-art", "prior_art_mapping", "Create prior-art search plan", "Write non-legal prior-art search targets and comparison dimensions.", writeFileCommand("evidence/prior-art-mapping.md", `# Prior-Art Mapping\n\nMission: ${mission.title}\n\nStatus: public-source adapter pending.\n\nSearch targets:\n- public repositories\n- papers and preprints\n- standards and protocol documentation\n- reproducibility and agent-evidence systems\n\nThis artifact does not make legal conclusions.\n`), "invention"),
    planStep("synthesize-candidate", "invention_synthesis", "Synthesize candidate", "Write a concrete candidate summary from the dossier scaffold.", writeFileCommand("evidence/invention-synthesis.md", `# Invention Synthesis\n\nCandidate: ${mission.title}\n\nMechanism: combine mission-scoped Node Alpha execution, command journaling, artifact scoring, and Sovryn publication gates so autonomous research can prepare artifacts without direct publication credentials.\n`), "invention"),
    planStep("skeptic-review", "skeptic_review", "Run skeptic review", "Record weaknesses before publication review.", writeFileCommand("evidence/skeptic-review.md", `# Skeptic Review\n\nRisks:\n- deterministic MVP does not prove novelty\n- prior-art mapping needs public-source adapters\n- Node Alpha local backend is not a sandbox\n- artifact quality depends on verification and review gates\n`), "invention"),
    planStep("prototype-test", "prototype_build", "Run prototype tests", "Execute the prototype validation suite.", "npm test", "prototype"),
    planStep("record-benchmark", "verification", "Record lightweight benchmark", "Write timing-free deterministic benchmark metadata for audit.", writeFileCommand("evidence/benchmark.json", JSON.stringify({ missionId: mission.id, benchmark: "prototype-test", status: "recorded", note: "MVP benchmark scaffold; no performance claim." }, null, 2)), "invention"),
    planStep("summarize-loop", "dossier_generation", "Summarize autonomous run", "Write a concise research-loop summary.", writeFileCommand("evidence/autonomous-summary.md", `# Autonomous Node Alpha Summary\n\nNode Alpha executed a deterministic research loop for ${mission.title}.\n\nOutputs include research-plan.json, command-journal.json, artifact-score.json, landscape notes, prior-art mapping, skeptic review, prototype verification, and benchmark metadata.\n`), "invention")
  ];
  const validationSteps = [
    planStep("node-version", "brief", "Check node", "Record Node.js version.", "node --version", "invention"),
    planStep("npm-version", "brief", "Check npm", "Record npm version.", "npm --version", "invention"),
    planStep("git-version", "brief", "Check git", "Record Git version.", "git --version", "invention"),
    planStep("prototype-test", "verification", "Run prototype tests", "Execute prototype validation.", "npm test", "prototype")
  ];
  return {
    nodeId,
    missionId: mission.id,
