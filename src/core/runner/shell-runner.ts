...
export class SshRunner implements RunnerAdapter {
  ...
  async run(input: RunnerInput): Promise<RunnerResult> {
    if (!this.config.policy.allowNetwork) {
      throw new AppError("NETWORK_BLOCKED", "SSH runner requires policy.allowNetwork=true.");
    }
    if (process.env.SOVRYN_SSH_PASSWORD || process.env.SSH_ASKPASS) {
      throw new AppError("PASSWORD_SSH_FORBIDDEN", "Password SSH is forbidden. Use SSH agent or an identity file.");
    }
    const ssh = this.config.runner.ssh;
    if (!ssh?.host) throw new AppError("SSH_HOST_REQUIRED", "SSH runner requires runner.ssh.host.");
