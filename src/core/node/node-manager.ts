...
export class NodeManager {
  ...
  async register(id: string, options: { host: "local" | string }): Promise<{ registration: NodeRegistration }> {
    await this.ensureInitialized();
    if (id !== "alpha") {
      throw new AppError("NODE_ID_UNSUPPORTED", "MVP supports Node Alpha as node id 'alpha'.", { id });
    }
    if (options.host !== "local") {
      throw new AppError("NODE_BACKEND_UNSUPPORTED", "MVP Node Alpha supports --host local. SSH, agentd, container, and VM backends are interface targets.", {
        host: options.host
      });
    }
    const now = nowIso();
  ...
  async logs(id: string, missionId: string): Promise<{ id: string; missionId: string; log: string }> {
    const registration = await this.readRegistration(id);
    assertNodeCapability(registration, "logs:stream");
    return { id, missionId, log: await this.backend(registration).readLogs(missionId) };
  }

  async artifacts(id: string, missionId: string): Promise<{ id: string; missionId: string; artifacts: NodeArtifactIndex }> {
    const registration = await this.readRegistration(id);
    assertNodeCapability(registration, "artifacts:collect");
    return { id, missionId, artifacts: await this.backend(registration).readArtifacts(missionId) };
  }
  ...
  private async readRegistration(id: string): Promise<NodeRegistration> {
    ...
    if (!registration) throw new AppError("NODE_NOT_REGISTERED", `Node not registered: ${id}`, { id });
    return registration;
  }

  private async readRegistrationOrNull(id: string): Promise<NodeRegistration | null> {
    try {
  ...
  private backend(registration: NodeRegistration): NodeAlphaBackend {
    if (registration.backend !== "local") {
      throw new AppError("NODE_BACKEND_UNSUPPORTED", "Only the local Node Alpha backend is implemented in this MVP.", {
