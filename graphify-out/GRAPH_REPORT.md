...
## Corpus Check
- 151 files · ~570,799 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2618 nodes · 8514 edges · 42 communities detected
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 1709 edges (avg confidence: 0.8)
...
1. `writeJson()` - 354 edges
2. `nowIso()` - 264 edges
3. `hashEvidence()` - 172 edges
4. `executeCli()` - 129 edges
...
Cohesion: 0.02
Nodes (89): configPath(), ensureGitignore(), initConfig(), readText(), discoverVerifyCommands(), exists(), readPackageJson(), AppError (+81 more)
...
Cohesion: 0.04
Nodes (68): averageEvidenceStrength(), buildBenchmarkPlan(), buildCandidateInventions(), buildClaimElementMap(), buildCounterEvidence(), buildExperimentPlan(), buildFactoryScore(), buildFactorySourceReadings() (+60 more)
...
Cohesion: 0.05
Nodes (65): appendLedger(), applyStagedCorpus(), autopublishRef(), average(), boolOrDefault(), candidateStatus(), clampInt(), copyExistingCorpusForStaging() (+57 more)

### Community 10 - "Community 10"
Cohesion: 0.06
Nodes (46): appendFactoryCandidateDocs(), assertFactoryEnabled(), boolOrDefault(), clampInt(), copyIfExists(), createFactoryId(), evidenceRefs(), exists() (+38 more)
...
- **Why does `executeCli()` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 14`, `Community 15`, `Community 16`, `Community 19`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Are the 353 inferred relationships involving `writeJson()` (e.g. with `makeTargetRepo()` and `writeResult()`) actually correct?**
  _`writeJson()` has 353 INFERRED edges - model-reasoned connections that need verification._
- **Are the 263 inferred relationships involving `nowIso()` (e.g. with `.check()` and `.demo()`) actually correct?**
  _`nowIso()` has 263 INFERRED edges - model-reasoned connections that need verification._
- **Are the 170 inferred relationships involving `hashEvidence()` (e.g. with `replacePriorArtEvidence()` and `replaceSourceReadingsEvidence()`) actually correct?**
  _`hashEvidence()` has 170 INFERRED edges - model-reasoned connections that need verification._
- **Are the 92 inferred relationships involving `executeCli()` (e.g. with `createPublicBetaFixture()` and `createStrictRunWithoutSharedState()`) actually correct?**
  _`executeCli()` has 92 INFERRED edges - model-reasoned connections that need verification._